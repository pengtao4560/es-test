...
- memory = allocate();   // 1、分配对象内存空间 （老师通知：一排一座一小时后张三会坐在这里）
- instance(memory);   // 2、初始化对象 （座位已经是张三预定的了）
- instance = memory;  // 3、设置instance指向刚刚分配的内存地址，此时instance != null （1小时过去了，张三做到了座位上。名副其实）

但是我们通过上面的三个步骤，能够发现，步骤2 和 步骤3之间不存在 数据依赖关系，而且无论重排前 还是重排后，程序的执行结果在单线程中并没有改变，
因此这种重排优化是允许的。

多线程环境下，某次指令重排后可能变成了：

- memory = allocate();   // 1、分配对象内存空间 （班主任通知：一排一座一小时后张三会坐在这里）
- instance = memory;  // 3、设置instance指向刚刚分配的内存地址，此时instance != null，但是对象还没有初始化完成 （座位已经是张三预定的了，但是座位上还没有人坐）
